...
void AT_CMD_parser(char *pdata)
{
   ...
   */
      ...
      printf("fetch router ssid : %s length:%d",WF_SSID,strlen(WF_SSID));
    ...
    */
     ...
     memcpy(auth,pdata + 9,4);
       ...
       if(memcmp(auth,"OPEN",4) != 0){
          ...
          printf("fetch router password : %s length:%d",WF_PWD,strlen(WF_PWD));
  ...
  /**设置AP的加密参数
    ...
    */
     ...
     if(memcmp(auth,"OPEN",4) != 0){
        ...
        printf("fetch ap password : %s length:%d",WF_PWD,strlen(WF_PWD));
     }
     wifi_password_conf((uint8_t *)WF_PWD,strlen(WF_PWD));
   ...
   */ 
      ...
      printf("fetch ap ssid : %s length:%d",WF_SSID,strlen(WF_SSID));
      wifi_ssid_conf((uint8_t *)WF_SSID,strlen(WF_SSID));
      UDP_Send("+ok\r",4);
      wifi_conf_select = STEP_APMODE;   
   }
